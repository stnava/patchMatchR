...
```{r,message=FALSE,warning=FALSE}
imgmask = randomMask( getMask( img ), 5000 )
mtch = patchMatch( imgc, img, imgmask, fixedPatchRadius = 7  )
myMatches = matchedPatches( imgc, img, mtch, fixedPatchRadius = 7, verbose=F )
...
rrr = RANSAC(
  ...
  maxIterations = 2000, verbose=T, goodProportion = 1 )
...
match = deepPatchMatch(  img2, img, mask, mask2,
  movingPatchSize = psz, fixedPatchSize = psz, verbose = TRUE,
  block_name = 21 )
wsel = which( !is.na( match$matches ) )
layout( matrix(1:2,nrow=1, byrow=T) )
ss=sample(wsel,1)
plot( as.antsImage( match$ffeatures$patches[ss,,] ) , colorbar=F, doCropping=F )
sss = match$matches[ss]
